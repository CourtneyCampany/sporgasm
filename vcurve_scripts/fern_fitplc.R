...
legend(x=3.5, y=13,boxlabs, pch=21, pt.bg=trtcols, bty='n',cex=1, pt.cex = 1,
       inset=-.02)
text(x=2.1,y=10,  bquote(bold(P[50] == .(round(epi_p50[[2]],2)))), cex=1)
...
plot(hemi_epi, cicol=alpha(trtcols[2], .75), xlab="",ylim=c(0,12.5),
    ...
    px_ci_label = FALSE)
axis(side=1, labels=FALSE, tick=TRUE)
text(x=-.4, y=6, k_lab2, srt=90, xpd=NA)
text(x=2.1,y=10,  bquote(bold(P[50] == .(round(hemi_p50[[2]],2)))), cex=1)
...
text(x=2.1,y=10,  bquote(bold(P[50] == .(round(terr_p50[[2]],2)))), cex=1)
